...
There is no install and no account. It is just plain files you own, set up so they stay useful instead of going stale.
...
- Tells your agent never to overwrite what you already have: if a file exists, read it and add to it.

It is a prompt, not a program. Your agent follows it, so check the short report it hands back when it is done. It is written to handle both a messy old repo and an empty one.
